...
PROCESS_THREAD(init_system_proc, ev, data){
        ...
        while (1) {
                ...
                int temp_data,usage_data,avail_data,id_data;
                getTempData(&temp_data);
                getUsageData(&usage_data);
                getIdData(&id_data);
                getAvailData(&avail_data);
                sprintf((char *)buff_udp, "availability:%d,usage:%d",avail_data,usage_data);
