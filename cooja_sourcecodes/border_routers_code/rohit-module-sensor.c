...
void getSpeedData(int *data) {
    *data = rand() % 140;
}

void getAccData(int *data) {
    *data = rand() % 40;
}

void getDecData(int *data) {
    *data = rand() % 40;
}

void getSteerData(int *data) {
    *data = rand() % 180;
}
...
PROCESS_THREAD(init_system_proc, ev, data){
        ...
        while (1) {
                ...
                int speed_data,acc_data,dec_data,steer_data;
                getSpeedData(&speed_data);
                getAccData(&acc_data);
                getDecData(&dec_data);
                getSteerData(&steer_data);
                sprintf((char *)buff_udp, "speed:%d,acceleration:%d,decelaration:%d,steering:%d",speed_data,acc_data,dec_data,steer_data);
