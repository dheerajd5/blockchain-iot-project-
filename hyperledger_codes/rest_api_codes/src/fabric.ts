...
export const createWallet = async (): Promise<Wallet> => {
  ...
  const org4Identity = {
    credentials: {
      certificate: config.certificateOrg4,
      privateKey: config.privateKeyOrg4,
    },
    mspId: config.mspIdOrg4,
    type: 'X.509',
  };

  await wallet.put(config.mspIdOrg4, org4Identity);

  const org5Identity = {
    credentials: {
      certificate: config.certificateOrg5,
      privateKey: config.privateKeyOrg5,
    },
    mspId: config.mspIdOrg5,
    type: 'X.509',
  };

  await wallet.put(config.mspIdOrg5, org5Identity);
...
export const getContracts = async (
  ...
  const assetContract = network.getContract(chaincode);
